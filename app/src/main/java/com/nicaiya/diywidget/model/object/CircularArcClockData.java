...
public class CircularArcClockData extends ImageData {
    ...
    public void draw(Canvas canvas) {
        ...
        clockPaint.setMaskFilter(new BlurMaskFilter(3.0f, BlurMaskFilter.Blur.NORMAL));
        clockPaint.setColor(bezelColor);
        clockPaint.setAlpha(bezelAlpha);
        float strokeWidth = Math.max(((float) (bezelBorder - bezelRadius) * 2.0f), 2.0f);
        clockPaint.setStrokeWidth(strokeWidth);
        float strokehalfWidth = strokeWidth / 2.0f;
        float left = (float) (bezelBorder * 2) + strokehalfWidth;
        float right = (float) ((bezelBorder * 2) + 403) - strokehalfWidth;
        arcRect.set(left, left, right, right);
        clockCanvas.drawArc(arcRect, 0.0f, 0.0f, false, clockPaint);
        clockPaint.setColor(hourColor);
        clockPaint.setAlpha(hourAlpha);
        strokeWidth = Math.max(((float) (hourBorder - hourRadius) * 2.0f), 2.0f);
        clockPaint.setStrokeWidth(strokeWidth);
        strokehalfWidth = strokeWidth / 2.0f;
        left = (float) (hourBorder * 2) + strokehalfWidth;
        right = (float) ((hourBorder * 2) + 403) - strokehalfWidth;
        arcRect.set(left, left, right, right);
        clockCanvas.drawArc(arcRect, -90.0f, (float) (((calendar.get(Calendar.HOUR) * 360) / 12) + ((calendar.get(Calendar.MINUTE) * 30) / 60)), false, clockPaint);
        clockPaint.setColor(minuteColor);
        clockPaint.setAlpha(minuteAlpha);
        strokeWidth = Math.max(((float) (minuteBorder - minuteRadius) * 2.0f), 2.0f);
        clockPaint.setStrokeWidth(strokeWidth);
        strokehalfWidth = strokeWidth / 2.0f;
        left = (float) (minuteBorder * 2) + strokehalfWidth;
        right = (float) ((minuteBorder * 2) + 403) - strokehalfWidth;
        arcRect.set(left, left, right, right);
        clockCanvas.drawArc(arcRect, -90.0f, (float) ((calendar.get(Calendar.MINUTE) * 360) / 60), false, clockPaint);
        super.draw(canvas);
